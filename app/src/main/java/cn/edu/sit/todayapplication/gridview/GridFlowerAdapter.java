...
//        // 猜测：自动将tag的内容填充到view中
//        viewHolder.imageView.setImageResource(mData.get(i).getImageId());
//        viewHolder.textView.setText(mData.get(i).getName());
        Glide.with(mContext).load(mData.get(i)).centerCrop().placeholder(R.mipmap.ic_launcher).diskCacheStrategy(DiskCacheStrategy.NONE).into(viewHolder.imageView);
